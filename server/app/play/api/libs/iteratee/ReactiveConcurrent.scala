...
object ReactiveConcurrent {
  ...
  trait Channel[E] {
     ...
     */
    def push(item: E): Future[Ack] = { push(Input.El(item)) }
   ...
   * [[Concurrent.Channel]] that can be used to feed input into the iteratee.  However, note that
  ...
  def unicast[E](
    ...
    def apply[A](it: Iteratee[E, A]): Future[Iteratee[E, A]] = {
      ...
      val pushee = new Channel[E] {
        ...
        private def schedule[U](body: => U): U = body
        ...
        def push(item: Input[E]) = schedule {
          ...
          val prev = iteratee.single.swap(eventuallyNext.future)
          prev.onComplete {
          ...
          prev.map(_.fold(Cancel: Ack)(_ => Continue))(dec)
        }
      }
